...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function(db, callback) {
        db.collection('ase').insertOne(response, function(err, result) {
            assert.equal(err, null);
            console.log("Inserted a document into the aselab9 collection.");
            callback();
        });
    };
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('aselab9').updateOne({"data.response.venues[i].name"  },{$set:item}, function(err, result) {
            if(err)            {
                res.write(" Failed");
                res.end();            }
